...
import './TopBarProgressIndicator.css';
...
const TopBarProgressIndicator = ({ 
  ...
  // Get system name based on data source
  const getSystemName = () => {
    const systemMap = {
      'ncc': 'Financial',
    ...
    return systemMap[selectedDataSource?.toLowerCase()] || 'Analysis';
  };
  ...
  return (
    <div className="top-bar-progress">
      ...
      <div className="agent-activity">
        <span className="activity-text">{getAgentActivity()}{dots}</span>
        <span className="step-indicator">Step {stepNumbers[currentStep]} of 4</span>
      </div>
    </div>
  );
};
