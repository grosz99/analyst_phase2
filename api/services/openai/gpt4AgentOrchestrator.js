...
class GPT4AgentOrchestrator {
  ...
  async analyzeData(data, analysisType, userContext, identifier = 'default') {
    ...
    try {
      ...
      const sqlQuery = this.extractSQLCode(analysisText);
      
      // Execute SQL query to get actual results
      let resultsTable = null;
      let visualization = null;
      let pythonResults = null;
      
      // Execute SQL query - NO FALLBACKS per CLAUDE.md policy
      if (sqlQuery && sqlQuery.code) {
        console.log('🔍 Executing SQL query to generate results...');
        pythonResults = this.sqliteService.executeQuery(sqlQuery.code, sanitizedContext);
        
        if (pythonResults.success && pythonResults.results) {
          // Use SQL execution results for table and visualization
          resultsTable = this.createTableFromSQLResults(pythonResults);
          visualization = this.createVisualizationFromSQLResults(pythonResults, sanitizedContext);
        } else {
          // FAIL PROPERLY - no fallback logic allowed
          throw new Error(`SQL execution failed: ${pythonResults.error || 'Unknown execution error'}`);
        }
      } else {
        // FAIL PROPERLY - no fallback logic allowed  
        throw new Error('No executable SQL query generated by GPT-4.1');
      }
...
1. Generate EXECUTABLE SQL queries that directly answer the user's question
2. The SQL should query the NCC table to get specific results (not generic summaries)
3. Include the executable SQL query in \`\`\`sql code blocks
4. Focus on the user's actual question - if they ask for "top 5 offices in 2024", filter and rank accordingly
5. Use proper SQL syntax with SELECT, WHERE, GROUP BY, ORDER BY as needed

DATABASE SCHEMA:
- Table: NCC
- Contains business/financial data with columns for offices, dates, sales, revenue, etc.

EXPECTED OUTPUT FORMAT:
```sql
-- SQL query to answer: "${userContext}"
-- Example: SELECT office, SUM(revenue) as total_revenue 
--          FROM NCC 
...
[Your specific SQL query here]
```

Then provide your analysis interpretation explaining what the query does and what insights it reveals.`
    };
  ...
  extractSQLCode(text) {
    const codeBlocks = text.match(/```sql\n?([\s\S]*?)\n?```/g);
    if (codeBlocks && codeBlocks.length > 0) {
      const code = codeBlocks[0].replace(/```sql\n?/, '').replace(/\n?```/, '');
      return {
        code: code.trim(),
        executable: code.trim().length > 0
      };
    }
    return null;
  }
  ...
  createTableFromSQLResults(sqlResults) {
    if (!sqlResults || !sqlResults.success) return null;
    
    const { results, columns } = sqlResults;
    
    if (results && results.length > 0 && columns) {
      return {
        title: "SQL Query Results",
        columns: columns,
        data: results, // SQLite results are already in object format
        rows: results.map(row => columns.map(col => row[col])), // Also provide rows format
        totalRows: results.length,
        source: "SQLite execution results"
      };
    }
  ...
  createVisualizationFromSQLResults(sqlResults, userContext) {
    if (!sqlResults || !sqlResults.success) return null;
